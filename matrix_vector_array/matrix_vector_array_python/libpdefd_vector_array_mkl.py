...
class vector_array_base:
    ...
    def __init__(self, data = None, shape = None, data_np = None):
        self._data_as = None
        self.data = None
        if data is not None:
            if isinstance(data, self.__class__):
                data = data.to_numpy_array().copy()
                shape = data.shape
    ...
    def __itruediv__(self, a):
        if isinstance(a, self.__class__):
            ...
            mkl.cblas_dscal(self.c_length, c_double(1/a), self._data_as, 1)
    ...
    def reduce_min(self):
        """
        compute 2^x for vector x; then take absolute min
        """
        res_np = np.empty(self.shape)
        res = res_np.ctypes.data_as(POINTER(c_double))
        mkl.vdLinearFrac(self.c_length, res,
                         res, c_double(0), c_double(2),
                         c_double(0), c_double(1), res)
        mkl.vdPow(self.c_length, res,
                  self._data_as, res)
        pos = mkl.cblas_idamin(self.c_length, res, 1)
        return self._data_as[pos]
    ...
    def reduce_max(self):
        ...
        mkl.vdPow(self.c_length, res,
                  self._data_as, res)
        pos = mkl.cblas_idamax(self.c_length, res, 1)
        return self._data_as[pos]
    ...
    def kron_vector(self, data):
        ...
        for i in range(0, self.length):
            ...
            iy_np = np.arange(i* data.length, i * data.length + data.length )
...
def vector_array(param, dtype=None, shape = None, *args, **kwargs):
    ...
    if isinstance(param, list):
        param_np = np.array(param, dtype=float).ctypes.data_as(POINTER(c_double))
        res_np = np.empty(np.prod((len(param),)))
        res = res_np.ctypes.data_as(POINTER(c_double))
        mkl.cblas_dcopy(c_int(len(param)), param_np.ctypes.data_as(POINTER(c_double)), 1, res, 1)
        retval._data_as = res
...
def vector_array_zeros(shape, dtype=None):
    ...
    retval.length = np.prod(shape)
    retval.c_length = c_int(retval.length)
    res_np = np.empty(np.prod(shape))
    res = res_np.ctypes.data_as(POINTER(c_double))
    mkl.cblas_dscal(retval.c_length, c_double(0), res, 1)
    retval._data_as = res
    return retval
...
def vector_array_ones(shape, dtype=None):
    ...
    res_np = np.empty(np.prod(shape))
    res = res_np.ctypes.data_as(POINTER(c_double))
    mkl.vdLinearFrac(res.c_length, res,
                     res, c_double(0), c_double(1),
                     c_double(0), c_double(1), res)
    return retval
