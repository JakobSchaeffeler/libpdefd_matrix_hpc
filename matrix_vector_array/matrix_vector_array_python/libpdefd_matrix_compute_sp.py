...
"""
try:
    import libpdefd.matrix_vector_array_np.libpdefd_vector_array as libpdefd_vector_array
    import libpdefd.matrix_vector_array.libpdefd_matrix_setup as libpdefd_matrix_setup
except:
    import sys, os
    sys.path.append(os.path.dirname(__file__))
    import libpdefd_vector_array_np.libpdefd_vector_array as libpdefd_vector_array
...
class matrix_sparse:
    ...
    def __add__(self, data):
        if isinstance(data, matrix_sparse):
            return self._matrix_csr + data._matrix_csr
    ...
    def dot__DEPRECATED(self, data):
        ...
        if isinstance(data, libpdefd_vector_array_np.vector_array_base):
            """
            If input is an ndarray, return also an libpdefd_vector_array
            """
            d = self._matrix_csr.dot(data._data)
            assert isinstance(d, np.ndarray)
            return libpdefd_vector_array_np.vector_array(d)
    ...
    def dot_add__DEPRECATED(self, x, c):
        ...
        if isinstance(x, libpdefd_vector_array_np.vector_array_base):
