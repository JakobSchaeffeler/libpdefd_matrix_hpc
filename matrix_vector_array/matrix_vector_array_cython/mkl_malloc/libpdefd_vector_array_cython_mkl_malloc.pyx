...
cdef class vector_array_base:
    ...
    cdef vector_array_base add_vector(vector_array_base self, vector_array_base b):
        cdef double* res_ptr = <double*> mkl_malloc(self.length * sizeof(double), 512)
...
cdef vector_array_base vector_array_c(param):
    ...
    if isinstance(param, np.ndarray):
        ...
        np_arr = param
        param_ptr = &np_arr[0]
        a = <double*> mkl_malloc(<size_t> param.size * sizeof(double), 64)
        if a == NULL:
...
cdef vector_array_base vector_array_zeros_c(shape):
    ...
    param_ptr = &np_arr[0]
    a = <double*> mkl_malloc(retval.length * sizeof(double), 512)
