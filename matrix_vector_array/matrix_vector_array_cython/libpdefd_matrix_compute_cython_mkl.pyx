...
cdef class matrix_sparse:
    ...
    cdef setup(self, data, tuple shape):
        ...
        """
        cdef int[:] indptr =  matrix_csr.indptr
        cdef int[:] indices =  matrix_csr.indices
        cdef double[:] values = matrix_csr.data

        cdef sparse_matrix_t matrix_mkl

        suc = mkl_sparse_d_create_csr(&matrix_mkl, SPARSE_INDEX_BASE_ZERO, matrix_csr.shape[0], matrix_csr.shape[1],
                                    &indptr[0],
                                    &indptr[1],
                                    &indices[0],
                                    &values[0])
