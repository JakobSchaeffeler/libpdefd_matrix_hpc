...
cdef class vector_array_base:
    ...
    def __add__(self, a):
        if isinstance(a, self.__class__):
            return self.__class__(vector_array_base.add_vector(self, a))
        
        if isinstance(a, float):
            return self.__class__(vector_array_base.add_scalar(self, a))
        
        raise Exception("Unsupported type '"+str(type(a))+"'")

    cdef vector_array_base add_vector(self, vector_array_base a):
        cdef np.ndarray[double, ndim=1] x = self._data
        cdef np.ndarray[double, ndim=1] y = a._data
    ...
    cdef vector_array_base add_scalar(self, double a):
        cdef np.ndarray[double, ndim=1] x = self._data
        return self.__class__(x+a)


    def __iadd__(self, a):
        if isinstance(a, self.__class__):
            return vector_array_base.add_vector(self, a)
    ...
    def abs(self):
        return vector_array(np.abs(self._data))
...
def vector_array(param, dtype=None, *args, **kwargs):
    ...
    if isinstance(param, np.ndarray):
        retval._data = np.copy(param.reshape((param.size,)))
