...
ext_modules = [
    Extension(
        "libpdefd_vector_array_cython_mkl_blas_cpp",
        ["libpdefd_vector_array_cython_mkl_blas_cpp.pyx"],
        extra_compile_args=[ '-lmkl_intel_ilp64', '-lmkl_gnu_thread', '-I/usr/include', '-lmkl_core', '-lgomp', '-lpthread', '-lm', '-ldl',  '-DMKL_ILP64',  '-m64', '-fopenmp'],
        extra_link_args=['-I/usr/include', '-lmkl_intel_ilp64', '-lmkl_gnu_thread', '-lmkl_core', '-liomp5', '-lpthread', '-lm', '-ldl',  '-DMKL_ILP64',  '-m64', '-fopenmp', '-lnuma',],
    include_dirs =[sched_path, mkl_include, np_path]
        )
]

setup(
        name='libpdefd_vector_array_cython_mkl_blas_cpp',
