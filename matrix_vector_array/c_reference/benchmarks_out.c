...
int main(int argc, char**argv){
	...
	FILE *f_min; 
	FILE *f_max;
	sprintf(str_f, "blas_c_div_%d_%d.csv", N, cores);
	sprintf(str_f_min, "blas_c_div_%d_%d_min.csv", N, cores);
	sprintf(str_f_max, "blas_c_div_%d_%d_max.csv", N, cores);
	x_a = NULL;
	x_b = NULL;
   	y_a = NULL;
    	y_b = NULL;
	...
	for(int i = 0; i < 10; i++){
		...
		res = div_vector_blas(x_blas, y_blas);
	...
	sprintf(str_f, "blas_c_div_%d_%d_scalar.csv", N, cores);
	sprintf(str_f_min, "blas_c_div_%d_%d_scalar_min.csv", N, cores);
	sprintf(str_f_max, "blas_c_div_%d_%d_scalar_max.csv", N, cores);
	...
	fclose(f_min);
	fclose(f_max);
	sprintf(str_f, "vml_c_div_%d_%d.csv", N, cores);
	sprintf(str_f_min, "vml_c_div_%d_%d_min.csv", N, cores);
	sprintf(str_f_max, "vml_c_div_%d_%d_max.csv", N, cores);
	...
	for(int i = 0; i < 10; i++){
		clock_gettime(CLOCK_MONOTONIC, &start);
		res = div_vector_vml(x_vml, y_vml);
		clock_gettime(CLOCK_MONOTONIC, &end);
		mkl_free(res);
		diff = (end.tv_sec - start.tv_sec) + 1.0e-9 * (end.tv_nsec - start.tv_nsec);
	}
	...
	fclose(f_min);
	fclose(f_max);
	sprintf(str_f, "vml_c_div_%d_%d_scalar.csv", N, cores);
	sprintf(str_f_min, "vml_c_div_%d_%d_scalar_min.csv", N, cores);
	sprintf(str_f_max, "vml_c_div_%d_%d_scalar_max.csv", N, cores);
